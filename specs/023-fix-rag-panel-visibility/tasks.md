...
- [ ] T021 Confirm all four PRs are green (typecheck + unit tests); merge in dependency order: chat-types → chat-backend → workbench-frontend + chat-frontend (last two in parallel); delete all remote and local `023-fix-rag-panel-visibility` branches after merge; sync local `develop` in each repo
...
- [ ] T023 [P] Update Confluence User Manual page ([https://mentalhelpglobal.atlassian.net/wiki/spaces/UD/pages/8749070/User+Manual](https://mentalhelpglobal.atlassian.net/wiki/spaces/UD/pages/8749070/User+Manual)) with a section describing the RAGDetailPanel — capture a screenshot of the expanded panel in `https://workbench.dev.mentalhelp.chat` using the Playwright MCP and embed it inline; note which roles can see the panel and how to expand it (Constitution XI)
