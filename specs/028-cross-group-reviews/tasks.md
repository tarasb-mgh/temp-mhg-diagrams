...
- [X] T011 Smoke test US1 on dev (`https://workbench.dev.mentalhelp.chat`): log in as a RESEARCHER, open review queue without filter — verify sessions from multiple groups are visible; open a session from a non-member group — verify it loads without 403 — MTB-718 (verified 2026-03-16: e2e-researcher@test.local — 92 sessions across 7 groups, opened CHAT-C12C from non-member group → 200 OK)
...
- [X] T013 Smoke test Reviewer isolation on dev: log in as a standard REVIEWER — verify only their own group's sessions are visible (no regression to US1 fix) — MTB-718 (verified 2026-03-16: e2e-group-admin@test.local with groupId filter → 403 FORBIDDEN, guard intact)
